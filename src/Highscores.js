...
HighScores.prototype = {
  ...
  loadLocal: function (typ) {
    ...
    const namecache = 'lcdgame_local_' + this.gametitle + '_hs' + typ;

    // load from localstorage
    const sc = window.localStorage.getItem(namecache);
  ...
  saveLocal: function (plr, sc, lvl, typ) {
    ...
    const rec = { player: plr, score: sc, level: lvl };
  ...
  checkScore: function () {
    ...
    const sc = this.lcdgame.score;
    const lvl = this.lcdgame.level;
    const typ = this.lcdgame.gametype;

    if (sc > 0) {
      // input name
      const lastname = (window.localStorage.getItem('lcdgame_highscore_name') || '');
      let plr = prompt('New highscore, enter your name and press enter to submit or press cancel.', lastname);
  ...
  onFilterButton: function (dv) {
    if (dv.currentTarget.dataset) {
      const typ = parseInt(dv.currentTarget.dataset.gametype);
  ...
  buildHeaderHTML: function () {
    ...
    let str = '<h1 id="scoretitle">' + this.gametitle + '</h1>';
    ...
    for (let i = 0; i < this.gametypes.length; i++) {
      const btn = document.getElementById('filtertype' + i);
