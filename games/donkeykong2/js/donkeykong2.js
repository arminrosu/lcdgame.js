...
const WAIT_STARTANIM = 2; // mario+dk+chain appear
const WAIT_LOSEANIM = 3;
const WAIT_MOVEKEY1 = 4;
const WAIT_MOVEKEY2 = 5;
const WAIT_LOCKANIM = 6;
const WAIT_WINANIM1 = 7; // climb down
const WAIT_WINANIM2 = 8; // catch Donkey Kong, add points
...
donkeykong2.MainGame.prototype = {
  ...
  refreshGameSpeed: function () {
    ...
    const msecs = 600 - ((this.lcdgame.level - 1) * 50) - ((5 - this.chains.length) * 25); // ms lower is faster speed
  ...
  tryMoveDkJr: function (dir) {
    ...
    if ((move !== 0) && (coll !== '')) {
      const hit = this.lcdgame.shapeVisible(coll);
      // collision
      if (hit) {
    ...
    if (move !== 0) {
      ...
      const collUp = MoveCollide[this.dkjrpos].collide[0];
      // check any collision
      ...
      if ((dir === DIR_JUMP) && (MoveCollide[this.dkjrpos].move[1] === 0)) {
        ...
        const en = MoveCollide[this.dkjrpos].collide[4];
      ...
      if (dir === DIR_UP) {
        // top rope positions
        const rope = [25, 29, 33, 37].indexOf(this.dkjrpos);
        if (rope >= 0) {
          // check if key is at same position
          if (this.keypos - 4 === rope) {
            // remove index from array
            const idx = this.chains.indexOf(this.keypos);
  ...
  doWait: function (code) {
    ...
    const keyvis = (this.waitmode === WAIT_MOVEKEY1) || (this.waitmode === WAIT_MOVEKEY2);
  ...
  onTimerWait: function () {
    ...
    switch (this.waitmode) {
      ...
      case WAIT_LOSEANIM:
        ...
        if ((t <= 14) && ((t + 1) % 3 === 0)) {
          const o = ((t + 1) % 6 === 0); // on at 0ms 600ms 1200ms
      ...
      case WAIT_MOVEKEY2:
        ...
        if (t === 10) {
          ...
          const idx = this.lcdgame.randomInteger(0, this.chains.length - 1);
  ...
  onTimerChance: function () {
    // blink lives on/off
    const b = (this.chancetimer.counter % 2 === 0);
  ...
  onTimerSpark: function () {
    // idle time out; if player does nothing, push DkJr into the game
    const t = (this.sparktimer.counter % 2 === 0);
    if (this.spark1 === true) {
  ...
  onTimerKey: function () {
    ...
    if (this.chancetime) {
      const s = (this.keytimer.counter % 2 === 0 ? this.lcdgame.score : '');
      this.refreshScore(s);
    }
  },
  ...
  onTimerGame: function () {
    // update moving enemies
    const t = this.gametimer.counter;

    // different pulse sound when on top screen
    const pls = ((t % 2 === 0) && (this.dkjrpos > 19) ? 'pulse2' : 'pulse1');
    this.lcdgame.playSoundEffect(pls);

    // check collisions
    let hit = false;
    const collUp = MoveCollide[this.dkjrpos].collide[0];
    const collDown = MoveCollide[this.dkjrpos].collide[1];
    const collLeft = MoveCollide[this.dkjrpos].collide[2];
    const collRight = MoveCollide[this.dkjrpos].collide[3];
