...
mariobros.ClockMode.prototype = {
  ...
  onTimerDemo: function (tmr) {
    ...
    let move = this.lcdgame.randomInteger(0, 1);
    move = (move === 0 ? -1 : +1);
...
mariobros.MainGame = function (lcdgame) {
  ...
  for (let c = 0; c < 8; c++) {
    const seq = (c % 2 === 0 ? 'case12' : 'case13');
    const max = (c % 2 === 0 ? 5 : 4) - Math.floor(c / 2);
...
mariobros.MainGame.prototype = {
  ...
  onTimerBelt: function () {
    ...
    let movebelt = (this.belttimer.counter % 4 === 0);
    const moveleft = (this.belttimer.counter % 8 === 0);

    // truck smoke animation, only when almost ready locading cases
    if (this.truckcases >= 6) {
      const smoke = (this.belttimer.counter % 4); // 0..3
      ...
      for (let c = 11; c > 0; c--) {
        ...
        if (docheck) {
          if ([2, 4, 6, 8, 10].indexOf(c) > -1) {
            ...
            let hold = this.lcdgame.sequenceShapeVisible(('case' + c), -1);
            this.holdcase[c] = hold;
            // cases going up one level (grabbed by mario or luigi)
            hold = this.holdcase[c - 1];

            if (hold) {
              this.lcdgame.sequenceSetPos(('case' + c), 0, false); // make empty so hold in place for one tick
            }
          }
  ...
  onTimerReset: function (tmr) {
    ...
    const m = (tmr.counter % 2 === 0 ? this.misscount : 0);
    this.refreshMiss(m);
  ...
  onTimerMiss: function () {
    const frame = this.misstimer.counter;
    if (frame === 1) {
      ...
      // bow down or up
      const bowdown = ((frame === 10) || (frame === 16));
  ...
  onTimerAnimate: function () {
    const frame = this.animatetimer.counter;
    ...
    if (frame < 30) {
      ...
      let smoke = frame + this.smokeframe;
