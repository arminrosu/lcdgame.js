...
const WAIT_LEVELANIM = 1;
const WAIT_LOSEANIM = 2;
const WAIT_ROOSTER = 3;
const WAIT_RESETANIM = 4;
...
eaglechicken.ClockMode.prototype = {
  ...
  updateClock: function () {
    ...
    if (this.timemode === 0) {
      ...
      const imonth = 1 + datenow.getMonth();
      const iday = datenow.getDate();
  ...
  updateDemo: function () {
    ...
    const r = this.lcdgame.randomInteger(0, 3);
...
eaglechicken.GameMode1.prototype = {
  ...
  press: function (btn) {
    ...
    if (this.gamestate === STATE_GAMEOVER) {
      // restart game
      if (btn === 'start') {
        const str = 'gamemode' + this.lcdgame.gametype;
  ...
  onTimerGame: function () {
    ...
    const dropbit = (this.eagledrop >> this.eaglepos) & 1;
    if (dropbit) {
      // drop in current eagle pos, or two places ahead/behind, i.e. at 1&3 or at 2&4
      const r = this.lcdgame.randomInteger(0, 1);
  ...
  onTimerWait: function () {
    const t = this.waittimer.counter;
  ...
  scorePoints: function (pts) {
    ...
    // display score
    const d = this.lcdgame.score % 10000; // display 9999 max? haven't tried such highscores
...
eaglechicken.GameMode2.prototype = {
  ...
  refreshScoreSecs: function () {
    const p = (this.countdown >= 0 ? this.countdown : 40 - this.countdown);
    const str = ('  ' + this.seconds).substr(-2) + ('00' + p).substr(-2);
