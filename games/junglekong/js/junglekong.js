...
const WAIT_LOSEANIM = 2;
const WAIT_BRIDGE = 3;
const WAIT_WINANIM = 4;
...
junglekong.ClockMode.prototype = {
  ...
  updateDemo: function () {
    ...
    // random new barrels
    const i = this.lcdgame.randomInteger(0, 5);
...
junglekong.SelectMode.prototype = {
  ...
  press: function (btn) {
    ...
    if (btn === 'dpad_left' || btn === 'dpad_right') { // dpad 2=left 3=right
      ...
      // show highscore
      const sc = this.lcdgame.highscores.getHighscore(this.lcdgame.gametype);
...
junglekong.MainGame.prototype = {
  ...
  continueGame: function () {
    ...
    const msecs = 820 - ((this.lcdgame.level - 1) * 15); // ms lower is faster speed
  ...
  press: function (btn, idx) {
    ...
    if (this.waitmode === STATE_GAMEOVER) {
      if (btn === 'dpad_left' || btn === 'dpad_right') {
        // show highscore before when selecting game A or game B
        const gamtyp = (btn === 'dpad_left' ? 1 : 2); // 1=game a, 2=game b
  ...
  tryMoveGuy: function (dir) {
    ...
    if (move !== 0) {
      ...
      if (this.jumping) {
        ...
        const i = ((this.guypos >= 9) && (this.guypos <= 15) ? 1 : 0); // exception middle platform, jump over right-barrels instead of left-barrels
  ...
  onTimerBlink: function () {
    const t = (this.blinktimer.counter % 2 === 0);
  ...
  onTimerSecond: function () {
    ...
    const t = this.sectimer.counter;
    if ((t % 3) === 0) {
      // update random vines
      const i = this.lcdgame.randomInteger(0, 7);
    ...
    if (this.lcdgame.gametype === 2) {
      ...
      if (this.birdpos < 4) {
        const frm = 'bird_' + (this.birdpos + 1);
  ...
  onTimerGame: function () {
    ...
    const hit = false;
    let collLeft = MoveCollide[this.guypos].collide[0];
    let collRight = MoveCollide[this.guypos].collide[1];
    ...
    if (this.gorillawait > 0) {
      ...
      if (this.gorillathrow === 2) {
        ...
        if (this.gorillathrow === 1) {
          ...
          const b = this.gorillapos - 1; // 0 or 1
        ...
        const r = this.lcdgame.randomInteger(0, 7);
