...
// constants
const STATE_DEMO = 1;
const STATE_MODESELECT = 2;
const STATE_GAMESTART = 10;
const STATE_GAMEPLAY = 11;
const STATE_GAMEPICK = 12;
const STATE_GAMEDROP = 13;

const STATE_GAMECRASH = 20;
const STATE_GAMEOVER = 21;
...
highway.ClockMode.prototype = {
  ...
  updateDemoRoad: function () {
    ...
    for (let i = 0; i < this.objfreqs.length; i++) {
      const o = this.lcdgame.randomInteger(1, 100);
    ...
    if ((this.objfreqs[1] === 1) && (this.objfreqs[2] === 1) && (this.objfreqs[3] === 1)) {
      const idx = this.lcdgame.randomInteger(1, 3);
...
highway.SelectMode.prototype = {
  ...
  press: function (btn) {
    ...
    if ((btn === 'left') || (btn === 'right')) {
      // show highscore
      const sc = this.lcdgame.highscores.getHighscore(this.selectdiff);
...
highway.MainGame.prototype = {
  ...
  onTimerCrash: function () {
    ...
    const b = (this.crashtimer.counter % 2 === 0);
    const p = (this.carpos % 5) - 1;
  ...
  updateRoad: function () {
    ...
    // score count
    const girl = this.lcdgame.sequenceShapeVisible('girl', 3);
    const dog = this.lcdgame.sequenceShapeVisible('dog', 3);
    const sign = this.lcdgame.sequenceShapeVisible('sign', 3);
    const tree = this.lcdgame.sequenceShapeVisible('tree', 3);
    const drop = this.lcdgame.sequenceShapeVisible('signdrop', 3);
...
// bonus game
// =============================================================================
const STATE_BONUSSTART = 0;
const STATE_BONUSPLAY = 1;
const STATE_BONUSEND = 2;
...
highway.BonusGame = function (lcdgame) {
  ...
  for (let d = 0; d < 3; d++) { // all 3 digit placeholders
    for (let e = 0; e < this.bonusfreqs[d].length; e++) { // all 3 digit placeholders
      const c = this.bonusfreqs[d][e].symbol;
...
highway.BonusGame.prototype = {
    ...
    switch (this.bonusstate) {
      case STATE_BONUSSTART:
        ...
        if ((this.waittimer.max > 1) && (this.waittimer.counter <= 3)) {
          ...
          const str = this.bonusvars[0].digit + this.bonusvars[1].digit + this.bonusvars[2].digit;
    ...
    for (let d = 0; d < 3; d++) { // all 3 digit placeholders
      ...
      if (d >= this.bonushold) {
        ...
        if (this.bonusvars[d].countdown <= 0) {
          ...
          const index = this.bonusvars[d].index;
  ...
  onTimerGas: function () {
    const onoff = (this.gastimer.counter % 2 === 0);
