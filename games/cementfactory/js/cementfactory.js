...
const WAIT_LIFT_MISS = 2;
const WAIT_CEMENT_MISS = 3;
...
cementfactory.ClockMode.prototype = {
  ...
  press: function (btn) {
    ...
    if ((btn === 'game_a') || (btn === 'game_b')) {
      ...
      // show highscore
      const sc = this.lcdgame.highscores.getHighscore((btn === 'game_a' ? 1 : 2));
  ...
  updateDemo: function () {
    ...
    const t = this.demotimer.counter;
    ...
    const n = this.lcdgame.randomInteger(1, 5);
    if (n === 1) {
      this.mario = 1 - this.mario; // toggle 0 and 1

      const b = (this.mario === 0);
  ...
  updateclock: function () {
    ...
    let ihours = datenow.getHours();
    const imin = datenow.getMinutes();
    ...
    const strtime = ('  ' + ihours).substr(-2) + ('00' + imin).substr(-2);
...
cementfactory.MainGame.prototype = {
  ...
  newGame: function () {
    ...
    const gametxt = (this.lcdgame.gametype === 1 ? 'game_a' : 'game_b');
  ...
  press: function (btn) {
    ...
    if (this.waitmode === STATE_GAMEOVER) {
      if ((btn === 'game_a') || (btn === 'game_b')) {
        // this.lcdgame.setShapeByName("time_colon", false);
        // show highscore
        const sc = this.lcdgame.highscores.getHighscore((btn === 'gamea' ? 1 : 2));
        this.lcdgame.digitsDisplay('digits', '' + sc, true);
      }
  ...
  refreshGameSpeed: function () {
    ...
    let msec = (this.lcdgame.gametype === 1 ? 750 : 688); // Game A starts at 750, Game B at 688
    if (this.lcdgame.gametype === 1) {
      // Game A
  ...
  tryMoveMario: function (dir) {
    ...
    if (move !== 0) {
      ...
      const lift = MoveCollide[this.mariopos].lift[0];
      if (lift > 0) {
        // check if lift is visible
        const liftpos = MoveCollide[this.mariopos].lift[1];
        const frm = 'lift' + lift + '_' + liftpos;
  ...
  tryMarioLever: function () {
    ...
    const arm = MoveCollide[this.mariopos].move[4];
    if (arm > 0) {
      ...
      const c = this.lcdgame.shapeVisible('cement_hopper' + arm + '_3');
      if (c) {
        // drop cement
  ...
  doSpillCement: function (buck) {
    ...
    const i = this.spillcement[buck - 1];
  ...
  onTimerBonus: function (tmr) {
    ...
    const m = (tmr.counter % 2 === 0 ? this.misses : 0);
    this.refreshMiss(m);
  ...
  onTimerChance: function (tmr) {
    ...
    const s = (tmr.counter % 2 === 0 ? this.lcdgame.score : '');
    this.refreshScore(s);
  },

  onTimerLift: function () {
    // update lifts
    const t = this.lifttimer.counter - 1;

    const n = this.lcdgame.randomInteger(0, 2);
    const onlift = MoveCollide[this.mariopos].lift[0];
    ...
    if ((t + 2) % 4 === 0) {
      ...
      if (this.liftvars.wait2 <= 0) {
        ...
        if (this.liftvars.pattern === 0) {
          ...
          const r = this.lcdgame.randomInteger(1, this.liftvars.chancemax);
    ...
    for (let h = 1; h <= 4; h++) {
      ...
      if ((i >= 0) && (i === (t % 4))) {
        ...
        for (let s = 3; s > 1; s--) {
          ...
          const shp3 = 'cement_hopper' + h + '_' + s;
          const shp2 = 'cement_hopper' + h + '_' + (s - 1);
  ...
  checkFullWarning: function () {
    // check all spills
    const full1 = this.lcdgame.sequenceAllVisible('cement_hopper1', true);
    const full2 = this.lcdgame.sequenceAllVisible('cement_hopper2', true);
  ...
  onTimerBucket: function () {
    // update buckets
    const t = this.bucktimer.counter;
