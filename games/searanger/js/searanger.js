...
// constants
const STATE_GAMEPLAY = 0;
const STATE_GAMEHIT = 1;
const STATE_GAMEBONUS = 2;
...
searanger.ClockMode.prototype = {
  ...
  onTimerDemo: function () {
    ...
    if ((this.demotimer.counter % 2) === 0) {
      ...
      const r1 = this.lcdgame.randomInteger(1, 100); // 1..100
      const r2 = this.lcdgame.randomInteger(1, 100); // 1..100
      const r3 = this.lcdgame.randomInteger(1, 100); // 1..100
      const r4 = this.lcdgame.randomInteger(1, 100); // 1..100
...
searanger.SelectMode.prototype = {
  ...
  press: function (btn) {
    ...
    if (btn === 'start') {
      ...
      const sc = this.lcdgame.highscores.getHighscore(this.gamepro);
...
searanger.MainGame.prototype = {
  ...
  onTimerGame: function () {
    ...
    if (this.guypos === 0) {
      ...
      const r = this.lcdgame.randomInteger(0, 2);
      const str = (r === 1 ? 'scorelong' : 'scoremedium');
    ...
    // waves animation
    const waves = (this.gametimer.counter + this.waveoffset) % 8;
    ...
    const frame = this.gametimer.counter % 4;
    ...
    // check if hit or avoid
    let hit = false;
    const last = this.lcdgame.sequenceShapeVisible(seq, -1);
    if (last) {
      // check if hit
  ...
  onTimerBuoy: function () {
    ...
    // etc.
    const frame = this.buoytimer.counter % 16;
  ...
  onTimerHit: function () {
    ...
    const frame = this.hittimer.counter % 16;
  ...
  scorePoints: function (pts) {
    ...
    const sc1 = (this.lcdgame.score % 1000);
    const sc2 = ((this.lcdgame.score + pts) % 1000);
    let wait = false;
    ...
    // display score
    const str = ('000' + this.lcdgame.score).slice(-3);
